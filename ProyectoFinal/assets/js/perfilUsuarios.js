	...
	$('.perfil .glyphicon-pencil')
			.on(
					...
					function() {
						...
						if ($(this).parent().parent().hasClass('infoPersonal')) {
							...
							if ($(this).siblings('span').is(':hidden')) {
								...
								$(this)
										...
										.hide()
										.after(
												'<form class="col-xs-5 col-sm-4" action="" method="POST"><input class="form-control input-sm infoPersonalEdicion" name="'
														+ name
														+ '" value="'
														+ $(this).siblings(
																'span').text()
						...
						} else if ($(this).parent().parent().hasClass(
								'restoPerfil')) {
							if ($(this).siblings('div').has('p').is(':hidden')) {
								$('.biografiaEdicion').hide().parent()
										.siblings('div').children('p').text(
												$('.biografiaEdicion').val()
														.trim()).parent()
										.show();
								$('.biografiaEdicion').parent().submit()
										.remove();
							} else {
								$(this)
										.siblings('div')
										...
										.hide()
										.after(
												'<form class="col-xs-12" style="margin-top: 2%" action="" method="POST"><textarea rows="7" cols="50" class="form-control biografiaEdicion" name="biografia" wrap="off">'
														+ $(this).siblings(
																'div').has('p')
																.text()
														+ '</textarea></form>');
